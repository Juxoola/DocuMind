...
def build_file_context(nodes, notebook_id: str):
    ...
    for i, node in enumerate(nodes, 1):
        ...
        time_sec = meta.get("start") or meta.get("time")
        time_str = ""
        if time_sec is not None:
            m = int(time_sec // 60)
            s = int(time_sec % 60)
            time_str = f" (Время: {m} мин {s} сек)"
            
        page_str = ""
        if meta.get("page") is not None:
            page_str = f" (Страница: {meta.get('page')})"
        ...
        context_parts.append(f"[{i}] Файл «{fname}»{time_str}{page_str}:\n{text}")
...
def make_prompt(query: str, context_str: str) -> str:
    return (
        ...
        "3. ВНИМАТЕЛЬНО проверяй ВРЕМЯ (таймкоды) источников. Если пользователь спрашивает про конкретный отрезок (например, 'с 5 по 20 минуту'), используй ТОЛЬКО источники, время которых попадает в этот отрезок. Остальные полностью игнорируй.\n"
        "4. После прямого ответа приведи подробное объяснение на основе источников.\n\n"
