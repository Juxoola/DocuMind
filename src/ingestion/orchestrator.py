...
# Единственная точка входа — определяет тип файла и делегирует аудио/видео/текстовому процессору
...
def ingest_file(file_path, notebook_id, progress_cb=None, llm_settings=None,
    ...
    if ext in [".mp4", ".avi", ".mkv", ".mov"]:
        file_path = ensure_720p_video(file_path, progress_cb, cancel_check=cancel_check, notebook_id=notebook_id)
    ...
    if ext in [".mp4", ".avi", ".mkv", ".mov", ".mp3"]:
        return process_audio_video(file_path, images_dir, ext != ".mp3", progress_cb,
                                   llm_settings, cancel_check=cancel_check,
                                   notebook_id=notebook_id, keep_vision_alive=keep_vision_alive,
                                   keep_whisper_alive=keep_whisper_alive)

    shared_llm_url = None
    try:
        if ext == ".pdf":
            nodes = process_pdf(file_path, images_dir, llm_settings, shared_llm_url,
                                progress_cb=progress_cb, cancel_check=cancel_check,
                                keep_vision_alive=keep_vision_alive)
        elif ext == ".pptx":
            nodes = process_pptx(file_path, images_dir, llm_settings, shared_llm_url,
                                 progress_cb=progress_cb, cancel_check=cancel_check,
                                 keep_vision_alive=keep_vision_alive)
        elif ext == ".docx":
            nodes = process_docx(file_path, images_dir, llm_settings, shared_llm_url,
                                 progress_cb=progress_cb, cancel_check=cancel_check,
                                 keep_vision_alive=keep_vision_alive)
        else:
            try:
