...
_http_session.mount(
    ...
    requests.adapters.HTTPAdapter(
        pool_connections=config.HTTP_POOL_SIZE_INGEST, pool_maxsize=config.HTTP_POOL_SIZE_INGEST
    ),
)
_http_session.mount(
    "https://",
...
def process_audio_video(
    ...
    if is_video:
        ...
        try:
            ...
            cancel_check_every = (
                5  # F-fix #video-cancel: проверяем cancel каждые ~5 кадров (~5с видео)
            )
            frame_counter = 0
            while True:
        ...
        prog(
            65,
            f"Описание {n} кадров ({'параллельно' if (int(llm_settings.get('vision_concurrency') or config.VISION_CONCURRENCY)) > 1 else 'последовательно'})...",
        )
        v_conc = int(llm_settings.get("vision_concurrency") or config.VISION_CONCURRENCY)
        ...
        with ThreadPoolExecutor(max_workers=v_conc) as executor:
            ...
            for idx, (path, t) in enumerate(frame_list):
                futures.append(
                    executor.submit(
                        describe_image_with_lmstudio, path, llm_settings, shared_llm_url
                    )
                )
...
def process_pdf(
    ...
    if n_workers <= 1:
        ...
        for page_num in range(total_pages):
            if _is_cancelled():
                logger.info(f"[Ingestion] Отмена на странице {page_num + 1}/{total_pages}")
        ...
        with ThreadPoolExecutor(max_workers=n_workers) as ex:
            ...
            for page_num in range(total_pages):
                if submitted % cancel_check_every == 0 and _is_cancelled():
                    logger.info(
                        f"[Ingestion] Отмена во время разбора страниц ({submitted}/{total_pages})"
                    )
            ...
            with ThreadPoolExecutor(max_workers=v_conc) as executor:
                ...
                try:
                    for future in as_completed(futures):
                        if _is_cancelled():
                            logger.info(
                                f"[Ingestion] Отмена во время OCR ({done_count}/{n} готово)"
                            )
...
def _safe_print(msg):
    ...
    logger.info()-ит в старой кодировке → битый вывод). Вместо этого пишем
...
def ensure_720p_video(file_path, prog_cb=None, cancel_check=None, notebook_id=None):
    ...
    if duration == 0:
        _safe_print(
            "[ensure_720p_video] WARNING Длительность неизвестна -> турбо-режим по умолчанию"
        )
        use_turbo = True
    else:
    ...
    if not use_turbo:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as ex:
            ...
            for p in ex.map(encode_seg, range(num_workers)):
                if _is_cancelled():
                    _safe_print(
                        f"[Ingestion] Отмена во время турбо-кодирования видео (получено {len(parts)}/{num_workers} сегментов)"
                    )
                    ex.shutdown(wait=False, cancel_futures=True)
