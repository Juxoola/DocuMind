...
def describe_image_with_lmstudio(image_path, llm_settings=None, existing_llm_url=None):
    ...
    prompt = """Проведи анализ изображения.
1. ГРАФИКА: Если есть схемы, диаграммы или таблицы — детально опиши их структуру и связи.
2. ТЕКСТ: Если на странице НЕТ сложных схем, но есть текст (например, скан или подписи) — выполни полный OCR этого текста с сохранением структуры.
3. КОНТЕКСТ: Кратко опиши суть страницы.
Пиши четко, без лишних вводных слов. Если есть и текст, и схемы — приоритет описанию схем."""
