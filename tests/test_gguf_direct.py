...
class TestDetectModelFamily:
    ...
    def test_gemma4_variants(self):
        ...
        from src.gguf_direct import detect_model_family
        for name in ["gemma-4-9b.gguf", "gemma4-9b.gguf", "gemma_4-9b.gguf",
                      "gemma-4b-Q4_K_M.gguf", "gemma-4e-Q8_0.gguf", "gemma_e4b.gguf"]:
...
class TestCacheTypeMap:
    ...
    def test_all_values_are_valid(self):
        ...
        for key, val in CACHE_TYPE_MAP.items():
            assert val in valid, (
                f"Ключ {key}: значение '{val}' не поддерживается llama-server. "
                f"Допустимы: {valid}"
            )
