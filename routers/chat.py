...
class ChatRequest(BaseModel):
    ...
    context_strategy: Optional[str] = "sliding"
...
async def chat(request: ChatRequest):
    ...
    print(f"DEBUG: Запрос чата. Стратегия контекста: {request.context_strategy}, "
        ...
        try:
            if request.image_base64 and use_direct_gguf:
                vision_messages = [{"role": "user", "content": [
                    ...
                    {"type": "text", "text": "Сделай точный OCR всего текста на изображении."}
                ]}]
                try:
                    v_payload = {"messages": vision_messages, "stream": False, "max_tokens": 1024}
                    r_vision = await asyncio.to_thread(
                        _http_session.post, f"{active_llm}/v1/chat/completions",
                        json=v_payload, timeout=60,
                    )
            ...
            if use_direct_gguf:
                ...
                messages_for_chat = [{"role": "system", "content": sys_prompt}, {"role": "user", "content": user_content}]
                ...
                async_gen = stream_gguf_chat(
                    llm_url=active_llm, messages=messages_for_chat,
                    enable_thinking=request.thinking_mode,
                ...
                prompt = make_prompt(request.query, context, thinking_mode=request.thinking_mode,
                                     max_tokens=request.max_tokens, answer_mode=request.answer_mode)
